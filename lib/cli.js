...
if (fs.existsSync(_serverDirectory) === false) {
  try {
    ...
    return callback(new Error("Cannot initialise the server directory: ", err.toString()));
  }
}
...
if (fs.existsSync(_commandsDirectory) === false) {
  try {
    ...
    return callback(new Error("Cannot initialise the server/commands directory: ", err.toString()));
  }
}
