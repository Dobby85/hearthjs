const converter = {
  ...
  _fillCurrent: function (model, current, obj) {
    ...
    if (columnName === 'TYPE_OBJECT') {
      ...
      if (hasFound === false) {
        ...
        if (this._checkAllFieldAreNotNullOrEmpty(toPush)) {
          current.push(toPush)
        }
      }
      return current
   ...
   */
  _checkAllFieldAreNotNullOrEmpty: function (obj) {
    let isNoNull = false

    for (let i = 0; i < Object.keys(obj).length; i++) {
      let key = Object.keys(obj)[i]

      if ((this._isArray(obj[key]) && obj[key].length !== 0) || (!this._isArray(obj[key]) && obj[key] !== null)) {
        isNoNull = true
      }
    }
