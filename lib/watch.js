...
let watchedDirectory = [
  ...
  /^\/addons\/\S*\.js$/, // Watch all addons files
  /^\/sql\/\S*\.js$/, // Watch all sql files
  /^\/helpers\/\S*\.js$/, // Watch all helpers files
  /^\/model\/\S*\.js$/, // Watch all model files
  /^\/datasets\/\S*\.sql$/ // Watch all model files
            ...
            if (watchedDirectory[j].test(path.join(directoryPath, files[i]))) {
              ...
              fs.watchFile(path.join(_completePath, files[i]), { persistent: true, interval: 500 }, (oldStat, newStat) => {
                if (newStat.mtime.getTime() !== oldStat.mtime.getTime()) {
                  return callbackStatus('change', files[i], path.join(_completePath, files[i]))
                }
              })
            }
          }
