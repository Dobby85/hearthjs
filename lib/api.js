...
const api = {
  ...
  _addRoute: function (method, route, schemaName) {
    ...
    // Find schema and add middleware on route
    let realSchema = this._apiList[this._currentApiName].schemas[schemaName]
    if (realSchema.middleware !== undefined) {
      realSchema.middleware.map((middleware) => {
        server._app.use(route, middleware)
      })
    }
    ...
    this._initAllAddons(Object.keys(realSchema), 0, route, realSchema, (err) => {
      ...
      if (realSchema.function === undefined) {
        ...
        server._app[method.toLowerCase()](route, func, (req, res) => {
        ...
        server._app[method.toLowerCase()](route, realSchema.function)
      }
