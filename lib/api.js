...
const api = {
  ...
  _addRoute: function (method, route, schemaName) {
    ...
    if (realSchema === undefined) {
      logger.log(`Schema ${schemaName} of API ${this._currentApiName} was not found`, 'warn', { mustLog: process.env.IS_LEADER_THREAD !== undefined || cluster.isMaster, logDate: false })
      return
    }
    ...
    this._initAllAddons(Object.keys(realSchema), 0, route, realSchema, (err) => {
      ...
      const _addonFunction = (req, res, next) => {
        ...
        this._execAddons(Object.keys(schema), 0, req, res, schema, (err, code) => {
          if (err) {
            code = code || 400
