 ...
 * @param {Function} callback
 */
function migrate (mode, options, callback) {
    ...
    database.init(databaseConf, (err) => {
      ...
      migration.init(serverPath, database, (err) => {
        ...
        migration.analyze((err) => {
          ...
          migration.getExecuteOrder(options.version, (err) => {
            if (err) {
              return callback(err)
            }
            ...
            askAllConflicts(migration._state, migration._toExecute, 0, totalConflict, 1, options, (err) => {
              if (err) {
                return callback(err)
              }
 ...
 * @param {Function} callback
 */
function askAllConflicts (states, toExecute, index, totalConflict, numberConflict, options, callback) {
  ...
  if (states[index].conflict && _toExecuteIndex !== -1) {
    return askOneConflict(states[index], totalConflict, numberConflict, options, (err) => {
      if (err) {
        return callback(err)
      }

      askAllConflicts(states, toExecute, index + 1, totalConflict, numberConflict + 1, options, callback)
    })
  }

  return askAllConflicts(states, toExecute, index + 1, totalConflict, numberConflict, options, callback)
}
 ...
 * @param {Function} callback
 */
function askOneConflict (state, totalConflict, numberConflict, options, callback) {
  // Get content in file
  fs.readFile(path.join(serverPath, 'migration', `${state.filenameId}_${state.conflictType}.sql`), 'utf8', (err, fileContent) => {
    ...
    database.query('SELECT "' + state.conflictType + '" FROM hearthjs."migration" WHERE "filename" = $1', [state.filenameId], (err, result) => {
      ...
      if (result.rows[0][state.conflictType].split('\n').length <= 5000) {
        diff = jsdiff.diffLines(result.rows[0][state.conflictType], fileContent)
      }
