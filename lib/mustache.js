...
const mustache = {
  ...
  render: function (template, data, model, sqlFiles, callback, defaultValues = { varIndex: 1, loopIndexes: [] }) {
    ...
    // this._readAllIncludes(tokens, 0, {}, sqlFiles, (err, includes) => {
    //   if (err) {
    //     return callback(err)
    //   }
    // })
  ...
  _buildString: function (tokens, data, varIndex, loopIndexes, model, sqlFiles, callback) {
    ...
    helper.genericQueue(tokens, (token, next) => {
      if (token[0] === 'text') {
        ...
        helper.genericQueue(indexArray, (i, next) => {
          loopIndexes.push(i)
  ...
  _buildTokens: function (tokens, index, currentList, returningTag) {
    ...
    for (var i = index; i < tokens.length; i++) {
      ...
      if (tagList.indexOf(tokens[i]) === -1) {
        ...
        } else if (isVarOpen || isLoopOpen || isCondOpen || isConstOpen || isIncludeOpen) {
          name += tokens[i]
        } else {
        ...
        currentList.push(['include', name.trim()])
        nbIncludeOpen -= 1
        name = ''
        isIncludeOpen = false
