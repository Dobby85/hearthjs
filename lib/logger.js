...
const logger = {
   ...
   */
  initLogger: function (env, parameters = { console_log: false }) {
    this._env = env
    this._consoleLog = parameters.console_log || false
  ...
  log: function (msg, level, options) {
    ...
    if (this._currentDate === this._getCurrentDateTime(false)) {
      ...
      if ((this._consoleLog === true || this._env !== 'prod') && level.toLowerCase() === 'error') {
        console.error(_logConsoleMsg)
      } else if (this._env === 'dev' || this._consoleLog) {
