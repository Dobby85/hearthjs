...
const validation = {
  ...
  _checkColumnName: function (schemaIn, data) {
    ...
    if (columnName === 'TYPE_OBJECT') {
      ...
      if (converter._isObject(schemaIn)) {
      ...
      let nbToSlice = (converter._isObject(schemaIn[0])) ? 1 : 2
      ...
      let schemaToUse = (converter._isObject(schemaIn[0])) ? schemaIn[0] : schemaIn[1]
    ...
    if (columnName === null) {
      for (let key in schemaIn) {
        ...
        if ((type === 'TYPE_KEY' && hasDefaultValue.has === false && data[key] === undefined) || (type !== 'TYPE_KEY' && data[key] === undefined)) {
          throw new Error(`Missing key ${key} in data`)
        } else if (type === 'TYPE_KEY' && hasDefaultValue.has === true) {
        ...
        if (type === 'TYPE_OBJECT') {
          ...
          let nbToSlice = (converter._isObject(schemaIn[key][0])) ? 1 : 2
          ...
          // Check how object was written [{}] => [0] / ['array', {}] => [1]
          let schemaToUse = (converter._isObject(schemaIn[key][0])) ? schemaIn[key][0] : schemaIn[key][1]
          result = this._validateObject(schemaToUse, data[key], true)
          newData[key] = result.newData
        } else if (type === null || type === 'TYPE_KEY') {
          // Validate value
          if (schemaIn[key].length > 0 && valueIsDefault === false) {
  ...
  _validateField: function (rules, data, key) {
    ...
    for (let i = 0; i < rules.length; i += 2) {
      ...
      if (comparaisonIndex !== -1) {
        ...
        if (rules[i + 1] === 'date') {
          let stringRegex = '^\\d{2}\\/\\d{2}\\/\\d{4}$'
          let regex = RegExp(stringRegex)

          if (data instanceof Date === false && regex.test(data) === false) {
            return {
              valid: false,
              message: this._getErrorMessage(rules, rules[i], `${key} has an invalid date format. It must be dd/mm/yyyy.`)
            }
          } else {
            if (moment(data, 'DD/MM/YYYY').isValid() === false) {
              return {
                valid: false,
                message: this._getErrorMessage(rules, rules[i], `${key} is an invalid date. ${data} does not exists.`)
              }
            }
          }
        } else {
