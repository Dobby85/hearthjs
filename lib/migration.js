...
let migration = {
  ...
  init: function (serverPath, database, callback) {
    ...
    /** Init the server/migration directory */
    helper._createDirectoryIfNotExists(this._directory)
  ...
  _readFileAndExecuteQuery: function (filename, callback) {
    fs.readFile(path.join(this._directory, filename), 'utf8', (err, content) => {
      ...
      this._database.query(content, (err) => {
        if (err) {
          return callback(new Error(`Error while applying script ${filename}: ${err.toString()} (${err.detail})`))
        }
