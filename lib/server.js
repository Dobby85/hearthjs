    ...
    this._loadConfig(env, options, (err) => {
      ...
      const _initFunction = (env, callback) => {
        this._init(env, (err) => {
          ...
          if (env === 'prod') {
            https.createServer({
              key: fs.readFileSync(path.join(this._serverPath, 'certificates', 'server.key')),
              cert: fs.readFileSync(path.join(this._serverPath, 'certificates', 'server.cert'))
            }, this._app)
            .listen(443, () => {
              const _logOptions = {
